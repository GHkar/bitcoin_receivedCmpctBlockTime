...
inline void static SendBlockTransactions(const CBlock& block, const BlockTransactionsRequest& req, CNode& pfrom, CConnman& connman) {
	...
	LogPrint(BCLog::NET, "KAR's Log : Received getBlockTxn tx=%s, block=%s, peer=%d, ip=%s\n", resp.txn[i]->GetHash().GetHex(), resp.blockhash.ToString(), pfrom.GetId(), pfrom.addr.ToString());
    }
    LOCK(cs_main);
    const CNetMsgMaker msgMaker(pfrom.GetSendVersion());
                                         ...
                                         const CChainParams& chainparams, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv &inv : vInv)
        {
            ...
            LogPrint(BCLog::NET, "KAR's Log : Received Inv got tx= %s, %s, peer=%d, ip=%s\n", inv.ToString(), fAlreadyHave ? "have" : "new", pfrom.GetId(), pfrom.addr.ToString());
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (pindex->nHeight <= ::ChainActive().Height() + 2) {
		    ...
		    LogPrint(BCLog::NET, "KAR's Log : Sending getBlockTxn index=");
		    for (size_t i = 0; i < req.indexes.size(); i++){	
			LogPrint(BCLog::NET, "%d ", req.indexes[i]);
		    }
	...
	{
		LogPrint(BCLog::NET, "KAR's Log : Received blockTxn tx=%s, block=%s, peer=%d\n", resp.txn[i]->GetHash().GetHex(), resp.blockhash.ToString(), pfrom.GetId());
	}
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
...

    {
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        vInv.push_back(inv);
			LogPrint(BCLog::NET, "KAR's Log : Sending Inv tx=%s, peer=%d, ip=%s\n", txid.ToString(), pto->GetId(), pto->addr.ToString());
        ...
        if (!vInv.empty()){
            m_connman.PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
	    LogPrint(BCLog::NET, "KAR's Log : Sending Inv End, peer=%d\n", pto->GetId());
	}
