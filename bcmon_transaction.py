rom bitcoinrpc.authproxy import AuthServiceProxy, JSONRPCException # bitcoinRPC
...
def collectData():
    ...
    best_height = 300000#bitcoin_rpc.getblockcount()
...
def executeSQL_tx(info, txindex, blockheight, blockhash, blocktime):
    ...
    # there is duplicate data, so replace it
        ...
        if 'Duplicate' in str(ex):
            ...
            sql = "DELETE FROM BVin2 WHERE Height=" + str(blockheight) + " AND TransactionIndex=" + str(txindex)

            cur.execute(sql)

            sql = "DELETE FROM BVout2 WHERE Height=" + str(blockheight) + " AND TransactionIndex=" + str(txindex)
