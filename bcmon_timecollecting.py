...
pattern_endTime = re.compile(r"(?P<date>\d+[-]\d+[-]\d+)T(?P<time>\d+[:]\d+[:]\d+[.]\d+)Z\s(Successfully\sreconstructed\sblock\s(?P<hash>\w+)\swith\s\d+\stxn\sprefilled\,\s\d+\stxn\sfrom\smempool\s\(incl\sat\sleast\s\d+\sfrom\sextra\spool\)\sand\s\d+\stxn\srequested)")
...
pattern_block_startTime = re.compile(r"(?P<date>\d+[-]\d+[-]\d+)T(?P<time>\d+[:]\d+[:]\d+[.]\d+)Z\sreceived\sblock\s(?P<hash>\w+)\speer\=\d+")
pattern_block_getHeight = re.compile(r"(?P<date>\d+[-]\d+[-]\d+)T(?P<time>\d+[:]\d+[:]\d+[.]\d+)Z\sUpdateTip[:]\snew\sbest\=(?P<hash>\w+)\sheight[=](?P<height>\d+)\sversion\=\w+\slog2[_]work\=\d+[.]\d+\stx\=\d+\sdate\=\'\d+[-]\d+[-]\d+T\d+[:]\d+[:]\d+Z\'\sprogress\=\d+[.]\d+\scache\=\d+[.]\d+MiB\(\d+txo\)")
			...
			try:
				...
				if is_cmpctBlock:
					...
					if get_startTime and get_endTime and get_height:
                        ...
                        make_db_json_cmpctblock(cmpct_height, receiveTime, stored_id, getblocktxnTime, gs_id, blocktxnTime, bs_id, reconstructedTime, res_id)
						print(db_json)
						reset_value()
						save_maria_anl_db()
						db_json = {}
					...
					if get_block_startTime and get_block_height:
						receiveTime = calculate_timeInterval(startTime, endTime)
						make_db_json(cmpct_height, receiveTime, stored_id)
						print(db_json)
						reset_value_block()
						save_maria_anl_db()
						db_json = {}
...
def save_maria_anl_db():
	...
	cur_anl.execute("INSERT INTO BCmpctTime_PerHeight VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE CmpctTime = ?, _id = ?", (int(db_json['Height']), db_json['CmpctTime'], int(db_json['_id']), db_json['CmpctTime'], int(db_json['_id'])))
...
def make_db_json_cmpctblock(cmpct_height, receiveTime, stored_id, getblocktxnTime, gs_id, blocktxnTime, bs_id, reconstructedTime, res_id):
	global db_json
...
# json 파일로 DB에 넣을 정보 저장
def make_db_json(height, receiveTime, stored_id):
	global db_json
	db_json['Height'] = height
	db_json['CmpctTime'] = receiveTime
	db_json['_id'] = stored_id
