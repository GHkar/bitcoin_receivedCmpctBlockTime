...
def connect_monogoDB():
    ...
    collection = mydb["log_SubNode_rtx_0506"]
...
def open_debug_log(collection):
    ...
    file = "\\\\210.125.29.157\\comnet\Computer Network Lab\\멤버\\재학생\\김애리\\연구\\비트코인 전달 시간 측정(코드, csv)\\코드\sub_debug_0506.log"
    f = open(file, "r") # 파일 열기
    f.seek(5)
    # 필요 변수
    count = 0
    ...
    while True:
        ...
        try:
            if pattern_received_inv.search(line) and not get_inv:
                ...
                if receivedTime > datetime.datetime(2021,5,5,14,50,00) and receivedTime < datetime.datetime(2021,5,6,15,00,00):
                    get_inv = True
                elif receivedTime > datetime.datetime(2021,5,6,15,00,00) :
                    print_result(count)
                    break
            ...
            if pattern_received_cmpctBlock.search(line) and not get_cmpctBlock:
                ...
                receivedTime = change_dateTime(find_line.group('date'), find_line.group('time'))
                if receivedTime > datetime.datetime(2021,5,5,15,00,00) and receivedTime < datetime.datetime(2021,5,6,15,00,00):
                    get_cmpctBlock = True
