...
def open_debug_log(mycol, stored_id):
   ...
   conn_anl, cur_anl = connectAEMariaDB()
         ...
         try:
            ...
            if is_cmpctBlock:
               ...
               if get_startTime and get_endTime and get_height:
                  ...
                  make_db_json_cmpctblock(cmpct_height, receiveTime, stored_id, getblocktxnTime, gs_id, blocktxnTime, bs_id, reconstructedTime, res_id, timestamp)

                  print(db_json)

                  reset_value()

                  save_maria_anl_db(cur_anl, conn_anl)
               ...
               if get_block_startTime and get_block_height:
                  ...
                  make_db_json(cmpct_height, receiveTime, stored_id, timestamp)
...
def save_maria_anl_db(cur_anl, conn_anl):
...
def connectRawMariaDB():
    conn_raw = maria_raw_connect(blockchain)
    cur_raw = conn_raw.cursor()

    return conn_raw, cur_raw
