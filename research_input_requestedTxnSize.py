...
pattern_receivedBlockTxn = re.compile(r"received:\sblocktxn\s\((?P<size>\d+)\sbytes\)\speer\=\d+")
...
def connect_monogoDB():
    ...
    collection = mydb["SubNode_cmpctblock_requetedTxnSize"]
...
def open_debug_log(collection):
    ...
    while True:
        ...
        try:
            ...
            if pattern_receivedBlockTxn.search(line) and get_hash:
            ...
            if get_height:
                ...
                make_db_json(cmpct_height, requestedTxnSize, receiveTime)
...
def make_db_json(height, requetedTxnSize, receiveTime):
    global db_json

    db_json['_id'] = height
    db_json['requestedTxnSize'] = requetedTxnSize
    db_json['receivedTime'] = receiveTime
