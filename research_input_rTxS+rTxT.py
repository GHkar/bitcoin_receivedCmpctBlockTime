## 요청한 트랜잭션 사이즈와 트랜잭션을 요청했을때 걸린 시간만 측정
...
pattern_sendingGetBlockTxn = re.compile(r"(?P<date>\d+[-]\d+[-]\d+)T(?P<time>\d+[:]\d+[:]\d+[.]\d+)Z\ssending\sgetblocktxn\s\((?P<size>\d+)\sbytes\)\speer\=\d+")
pattern_receivedBlockTxn = re.compile(r"(?P<date>\d+[-]\d+[-]\d+)T(?P<time>\d+[:]\d+[:]\d+[.]\d+)Z\sreceived:\sblocktxn\s\((?P<size>\d+)\sbytes\)\speer\=\d+")
...
get_startTime = False
get_endTime = False
get_sendingGetBlockTxn = False
get_receivedBlockTxn = False
get_height = False
get_hash = False
...
def connect_monogoDB():
    ...
    collection = mydb["SubNode_cmpctblock_requetedTxnSize+requestedTxnTime_newVer"]
...
def open_debug_log(collection):
    ...
    global get_sendingGetBlockTxn
    global get_receivedBlockTxn
    global get_endTime
    global get_height
    global get_hash
    global db_json
    ...
    while True:
        ...
        try:
            ...
            if pattern_sendingGetBlockTxn.search(line) and get_hash:
                find_line = pattern_sendingGetBlockTxn.search(line)
                startTime = change_dateTime(find_line.group('date'), find_line.group('time'))
                get_sendingGetBlockTxn = True

            if pattern_receivedBlockTxn.search(line):
                find_line = pattern_receivedBlockTxn.search(line)
                if not get_sendingGetBlockTxn:
                    endTime = startTime
                elif get_sendingGetBlockTxn:
                    endTime = change_dateTime(find_line.group('date'), find_line.group('time'))
                requestedTxnSize = int(find_line.group('size')) - 33
                get_receivedBlockTxn = True

            if pattern_endTime.search(line) and get_receivedBlockTxn:
                find_line = pattern_endTime.search(line)
                if cmpct_hash == find_line.group('hash'):
                    rTx = int(find_line.group('RTX'))
                    get_endTime = True
            ...
            if get_height:
                receiveTxnTime = calculate_timeInterval(startTime, endTime)
                make_db_json(cmpct_height, requestedTxnSize, receiveTxnTime, rTx)
...
def make_db_json(height, requetedTxnSize, receiveTxnTime, rTx):
    global db_json

    db_json['_id'] = height
    db_json['requestedTxnSize'] = requetedTxnSize
    db_json['receivedTime'] = receiveTxnTime
    db_json['requetedTxNum'] = rTx
